      ...
      {selectedCertificate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/85 backdrop-blur-md overflow-y-auto">
          ...
          <div className="relative flex flex-col items-center max-w-4xl w-full my-auto px-1 sm:px-0">
            ...
            <div id="certificate-container" className="relative w-full aspect-[1.4] bg-[#FDFBF7] shadow-2xl rounded-sm border-[8px] sm:border-[16px] border-[#0A192F] px-5 sm:px-12 py-5 sm:py-8 overflow-hidden text-neutral-900 flex flex-col justify-between">
              
              {/* Inner Gold Fine Line Border */}
              <div className="absolute inset-2 sm:inset-3 border border-[#C5A059]/60 sm:border-2 pointer-events-none z-20" />

              {/* Corner Ornaments */}
              <div className="absolute top-3 left-3 sm:top-4 sm:left-4 w-3 h-3 sm:w-4 sm:h-4 border-t-2 border-l-2 border-[#C5A059] z-20" />
              <div className="absolute top-3 right-3 sm:top-4 sm:right-4 w-3 h-3 sm:w-4 sm:h-4 border-t-2 border-r-2 border-[#C5A059] z-20" />
              <div className="absolute bottom-3 left-3 sm:bottom-4 sm:left-4 w-3 h-3 sm:w-4 sm:h-4 border-b-2 border-l-2 border-[#C5A059] z-20" />
              <div className="absolute bottom-3 right-3 sm:bottom-4 sm:right-4 w-3 h-3 sm:w-4 sm:h-4 border-b-2 border-r-2 border-[#C5A059] z-20" />

              {/* CENTER DRY SEAL WATERMARK */}
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-0 opacity-[0.07] select-none">
                <div className="w-[50vw] max-w-[400px] aspect-square rounded-full border-4 border-neutral-900 flex items-center justify-center p-8 transform rotate-12">
                  <img 
                    src="/1080.png" 
                    alt="Dry Seal Watermark" 
                    className="w-full h-full object-contain filter grayscale contrast-200"
                  />
                </div>
              </div>

              {/* Header Section - Pushed further down from the border, separated text */}
              <div className="relative z-10 text-center flex flex-col items-center pt-8 sm:pt-14">
                <p className="text-[8px] sm:text-[11px] font-bold uppercase tracking-[0.2em] sm:tracking-[0.25em] text-[#C5A080] whitespace-nowrap overflow-hidden text-ellipsis mb-3 sm:mb-5" style={{ fontFamily: "'Cinzel', serif" }}>
                  Ministry of Christ&apos;s Great Commission Church Inc.
                </p>
                <h1 className="text-lg sm:text-3xl lg:text-4xl font-black text-[#0A192F] tracking-wider whitespace-nowrap" style={{ fontFamily: "'Cinzel', serif" }}>
                  CERTIFICATE OF COMPLETION
                </h1>
                <div className="w-20 sm:w-32 h-0.5 bg-[#C5A059] mt-2 sm:mt-3" />
              </div>

              {/* Body Presentation */}
              <div className="relative z-10 text-center space-y-3 sm:space-y-5 my-auto">
                <p className="text-[8px] sm:text-xs uppercase tracking-widest text-neutral-500 font-semibold italic">
                  This proudly certifies that
                </p>

                {/* Student Name */}
                <div>
                  <p style={{ fontFamily: "'Great Vibes', cursive" }} className="text-4xl sm:text-6xl lg:text-7xl text-[#0A192F] leading-none pb-1">
                    {userProfile?.full_name || "Student Name"}
                  </p>
                  <div className="w-40 sm:w-72 h-px bg-neutral-300 mx-auto mt-2" />
                </div>

                {/* Course details flow */}
                <div className="space-y-1.5 sm:space-y-2 max-w-xl mx-auto">
                  <p className="text-[8px] sm:text-xs text-neutral-600 leading-relaxed px-2">
                    Has successfully completed all requirements, coursework, and spiritual milestones for the course:
                  </p>
                  <p className="text-[10px] sm:text-sm lg:text-base font-bold text-[#C5A059] uppercase tracking-wide" style={{ fontFamily: "'Cinzel', serif" }}>
                    {selectedCertificate.course_name}
                  </p>
                  <p className="text-[7px] sm:text-[10px] text-neutral-500 italic pt-0.5">
                    &ldquo;Let the word of Christ dwell in you richly, teaching and admonishing one another in all wisdom.&rdquo; &mdash; Colossians 3:16
                  </p>
                </div>
              </div>

              {/* Footer Section: Strict 3-Column Grid */}
              <div className="relative z-10 grid grid-cols-3 items-end pb-1 sm:pb-3 pt-3 border-t border-neutral-200">
                
                {/* Left: Date Issued */}
                <div className="flex justify-start">
                  <div className="w-24 sm:w-36 text-center space-y-0.5 sm:space-y-1">
                    <p className="text-[8px] sm:text-[11px] font-bold text-[#0A192F]" style={{ fontFamily: "'Cinzel', serif" }}>{selectedCertificate.issued_at}</p>
                    <div className="w-full h-px bg-neutral-400" />
                    <p className="text-[7px] sm:text-[9px] uppercase tracking-wider text-neutral-500 font-semibold">Date Issued</p>
                  </div>
                </div>

                {/* Center: Exactly Centered Church Logo */}
                <div className="flex justify-center">
                  <div className="w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 border-2 border-[#C5A059]/50 rounded-full flex items-center justify-center p-1 bg-white shadow-md">
                    <img 
                      src="/1080.png" 
                      alt="Church Logo" 
                ...
                {/* Right: Developer Message Quote */}
                <div className="flex justify-end">
                  <div className="w-32 sm:w-48 lg:w-56 text-center space-y-0.5 sm:space-y-1">
                    <p className="text-[7px] sm:text-[9px] lg:text-[10px] text-neutral-700 leading-tight italic px-1">
                      &ldquo;This is more than an achievement. It is another step in the journey God has prepared for you. Keep learning, keep believing, and keep going.&rdquo;
                    </p>
                    <div className="w-full h-px bg-neutral-300 mt-1" />
                    <p className="text-[7px] sm:text-[9px] uppercase tracking-wider text-amber-800 font-bold">A Message from the Developer</p>
                  </div>
                </div>
            ...
            {/* Close Indicator Note */}
            <div className="mt-4 text-center">
              <p className="text-xs text-slate-400">Click the X above to close the certificate viewer.</p>
            </div>
